...
export const useTripStore = create<TripState>()(
  persist(
    (set, get) => ({
      ...
      updateDayCoverImage: (tripId, dayIndex, imageUrl) => updateStateAndSave(set, get, state => ({ trips: state.trips.map(trip => { if (trip.id !== tripId) return trip; const newItinerary = [...trip.dailyItinerary]; if (newItinerary[dayIndex]) newItinerary[dayIndex] = { ...newItinerary[dayIndex], coverImage: imageUrl }; return { ...trip, dailyItinerary: newItinerary }; }) }), tripId),
