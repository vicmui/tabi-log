...
export interface DailyItinerary { day: number; date: string; weather?: string; activities: Activity[]; coverImage?: string; } // 🔥 新增每日封面
...
interface TripState {
  ...
  deleteTrip: (tripId: string) => void; // 🔥 新增刪除旅程
  ...
  updateDayCoverImage: (tripId: string, dayIndex: number, imageUrl: string) => void; // 🔥 新增每日封面更新
}
...
const INITIAL_TRIP: Trip = {
  ...
  dailyItinerary: [{ day: 1, date: "2026-03-20", weather: "Cloud", activities: [] }, { day: 2, date: "2026-03-21", weather: "Sun", activities: [] }, { day: 3, date: "2026-03-22", weather: "Sun", activities: [] }, { day: 4, date: "2026-03-23", weather: "Rain", activities: [] }, { day: 5, date: "2026-03-24", weather: "Cloud", activities: [] }]
};
...
export const useTripStore = create<TripState>((set, get) => ({
  trips: [], activeTripId: null, isSyncing: false,
  ...
  saveTripToCloud: async (trip: Trip) => { set({ isSyncing: true }); const { error } = await supabase.from('trips').upsert({ id: trip.id, title: trip.title, content: trip, updated_at: new Date().toISOString() }); if (error) alert(`同步失敗！\n錯誤: ${error.message}`); set({ isSyncing: false }); },
  ...
  deleteTrip: async (tripId) => {
    if(!confirm("確定要永久刪除此旅程嗎？此操作無法復原。")) return;
  ...
  updateBooking: (tripId, bookingId, data) => updateStateAndSave(set, get, state => ({ trips: state.trips.map(t => t.id === tripId ? { ...t, bookings: t.bookings.map(b => b.id === bookingId ? { ...b, ...data } : b) } : t) })),
  ...
  addExpense: (tripId, expense) => updateStateAndSave(set, get, state => ({ trips: state.trips.map(t => t.id === tripId ? { ...t, expenses: [expense, ...t.expenses] } : t) })),
  updateExpense: (tripId, expenseId, data) => updateStateAndSave(set, get, state => ({ trips: state.trips.map(t => t.id === tripId ? { ...t, expenses: t.expenses.map(e => e.id === expenseId ? { ...e, ...data } : e) } : t) })),
  deleteExpense: (tripId, expenseId) => updateStateAndSave(set, get, state => ({ trips: state.trips.map(t => t.id === tripId ? { ...t, expenses: t.expenses.filter(e => e.id !== expenseId) } : t) })),
  addPlanItem: (tripId, item) => updateStateAndSave(set, get, state => ({ trips: state.trips.map(t => t.id === tripId ? { ...t, plans: [...t.plans, item] } : t) })),
  ...
  deletePlanItem: (tripId, itemId) => updateStateAndSave(set, get, state => ({ trips: state.trips.map(t => t.id === tripId ? { ...t, plans: t.plans.filter(p => p.id !== itemId) } : t) })),
  ...
  // 🔥 新增：更新每日封面
  updateDayCoverImage: (tripId, dayIndex, imageUrl) => updateStateAndSave(set, get, state => ({
    trips: state.trips.map(trip => {
      if (trip.id !== tripId) return trip;
      const newItinerary = [...trip.dailyItinerary];
      if (newItinerary[dayIndex]) {
        newItinerary[dayIndex].coverImage = imageUrl;
      }
