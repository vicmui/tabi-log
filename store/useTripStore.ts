...
// ... (類型定義 Interfaces 保持不變，為節省篇幅省略，請保留之前的) ...
// 為了方便你 Copy，我這裡列出完整的 Store 結構，確保無漏
...
interface TripState {
  ...
  deleteTrip: (tripId: string) => Promise<void>; // 🔥 改為 Promise
...
export const useTripStore = create<TripState>((set, get) => ({
  ...
  saveTripToCloud: async (trip: Trip) => { set({ isSyncing: true }); const { error } = await supabase.from('trips').upsert({ id: trip.id, title: trip.title, content: trip, updated_at: new Date().toISOString() }); if (error) console.error("Sync Error", error); set({ isSyncing: false }); },
  ...
  deleteTrip: async (tripId) => {
    ...
    if(error) { 
        alert("刪除失敗 (Supabase Error): " + error.message); 
    } else {
        set(state => {
            const newTrips = state.trips.filter(t => t.id !== tripId);
            // 如果刪除了當前選中的 Trip，自動選下一個
            const newActiveId = state.activeTripId === tripId ? (newTrips.length > 0 ? newTrips[0].id : null) : state.activeTripId;
            return { trips: newTrips, activeTripId: newActiveId };
        });
    }
    set({ isSyncing: false });
  },
