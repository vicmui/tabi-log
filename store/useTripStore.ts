...
import { persist, createJSONStorage } from 'zustand/middleware'; // 🔥 重新加回本地持久化
...
export interface Booking {
  ...
  details: { price?: number; address?: string; fileUrl?: string; note?: string; airline?: string; flightNum?: string; seat?: string; gate?: string; origin?: string; destination?: string; departTime?: string; arriveTime?: string; checkIn?: string; checkOut?: string; pickupLocation?: string; dropoffLocation?: string; };
}
...
export const useTripStore = create<TripState>()(
  persist(
    (set, get) => ({
      ...
      updateTripRate: (tripId, rate) => { set(state => ({ trips: state.trips.map(t => t.id === tripId ? { ...t, exchangeRate: rate } : t) })); const updated = get().trips.find(t => t.id === tripId); if(updated) get().saveTripToCloud(updated); },
      ...
      addActivity: (tripId, dayIndex, activity) => { set(state => ({ trips: state.trips.map(trip => { if (trip.id !== tripId) return trip; const newItinerary = [...trip.dailyItinerary]; if (!newItinerary[dayIndex]) return trip; newItinerary[dayIndex].activities.push({ ...activity, id: uuidv4(), isVisited: false }); newItinerary[dayIndex].activities.sort((a, b) => a.time.localeCompare(b.time)); return { ...trip, dailyItinerary: newItinerary }; }) })); const updated = get().trips.find(t => t.id === tripId); if(updated) get().saveTripToCloud(updated); },
      updateActivity: (tripId, dayIndex, activityId, data) => { set(state => ({ trips: state.trips.map(trip => { if (trip.id !== tripId) return trip; const newItinerary = [...trip.dailyItinerary]; newItinerary[dayIndex].activities = newItinerary[dayIndex].activities.map(a => a.id === activityId ? { ...a, ...data } : a); if(data.time) newItinerary[dayIndex].activities.sort((a, b) => a.time.localeCompare(b.time)); return { ...trip, dailyItinerary: newItinerary }; }) })); const updated = get().trips.find(t => t.id === tripId); if(updated) get().saveTripToCloud(updated); },
    ...
    { 
        name: 'vm-build-v14-final-with-local', // 🔥 換名以強制更新並啟用本地持久化
