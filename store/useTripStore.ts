...
export interface PlaceToVisit { id: string; name: string; address?: string; note?: string; category?: string; lat?: number; lng?: number; isVisited: boolean; }
...
export const useTripStore = create<TripState>()(
  persist(
    (set, get) => ({
      ...
      updatePlanItem: (tripId, itemId, data) => updateStateAndSave(set, get, state => ({ trips: state.trips.map(t => t.id === tripId ? { ...t, plans: t.plans.map(p => p.id === itemId ? { ...p, ...data } : p) } : t) })),
      ...
      addActivity: (tripId, dayIndex, activity) => updateStateAndSave(set, get, state => ({ trips: state.trips.map(trip => { if (trip.id !== tripId) return trip; const newItinerary = [...trip.dailyItinerary]; if (!newItinerary[dayIndex]) return trip; const { cost, ...rest } = activity; newItinerary[dayIndex].activities.push({ ...rest, id: uuidv4(), isVisited: false, cost: 0 }); newItinerary[dayIndex].activities.sort((a, b) => a.time.localeCompare(b.time)); return { ...trip, dailyItinerary: newItinerary }; }) }), tripId),
