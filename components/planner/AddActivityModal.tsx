...
export default function AddActivityModal({ isOpen, onClose, onSubmit }: Props) {
  ...
  useEffect(() => {
    ...
    if (key) setApiKey(key); else setIsGoogleMode(false);
  }, []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({ type, time, location: locationName, cost, note: finalNote, lat, lng });
    setLocationName(""); setAddress(""); setCost(0); setNote(""); setLat(null); setLng(null); onClose();
  };
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed inset-0 m-auto w-full max-w-md h-fit bg-white z-[101] shadow-2xl p-8 rounded-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <div className="grid grid-cols-6 gap-2">{TYPES.map((t) => (<button key={t.type} type="button" onClick={() => setType(t.type)} className={clsx("flex flex-col items-center justify-center py-2 gap-1 rounded-lg transition-all border", type === t.type ? "bg-[#333333] text-white border-black" : "bg-white text-gray-400 border-gray-100 hover:border-gray-300")}><t.icon size={14} /></button>))}</div>
              </div>
              <div>
                 <div className="flex justify-between items-center mb-2">
                    <label className="block text-[10px] font-bold text-gray-400 tracking-widest uppercase">地點 / 店名</label>
                    {apiKey && (<button type="button" onClick={() => setIsGoogleMode(!isGoogleMode)} className="text-[10px] text-blue-500 underline">{isGoogleMode ? "切換手動" : "Google 搜尋"}</button>)}
                 </div>
                 {isGoogleMode && apiKey ? (
                 ...
                 ) : (<input type="text" placeholder="手動輸入..." value={locationName} onChange={(e) => setLocationName(e.target.value)} className="w-full border-b py-2 text-sm"/>)}
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-[10px] font-bold text-gray-400 tracking-widest mb-1 uppercase">時間</label><input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full border-b py-2 text-sm"/></div>
                <div><label className="block text-[10px] font-bold text-gray-400 tracking-widest mb-1 uppercase">預算 (¥)</label><input type="number" value={cost||''} onChange={(e) => setCost(Number(e.target.value))} className="w-full border-b py-2 text-sm" placeholder="0"/></div>
              </div>
              <div><label className="block text-[10px] font-bold text-gray-400 tracking-widest mb-1 uppercase">備註</label><input type="text" value={note} onChange={(e) => setNote(e.target.value)} className="w-full border-b py-2 text-sm" placeholder="選填..."/></div>
