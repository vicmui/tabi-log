        ...
        <>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed inset-0 m-auto w-full max-w-md h-fit bg-white z-[101] shadow-2xl p-8 ">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div><label className="block text-xs text-gray-400 mb-2">類別</label><div className="grid grid-cols-6 gap-2">{TYPES.map((t) => (<button key={t.type} type="button" onClick={() => setType(t.type)} className={clsx("flex flex-col items-center justify-center py-2 gap-1  border", type === t.type ? "bg-black text-white" : "bg-white text-gray-400 border-gray-100")}><t.icon size={14} /></button>))}</div></div>
              <div>
              ...
              <button type="submit" disabled={!customName && !googleAddress} className={clsx("w-full py-3  text-xs font-bold uppercase mt-2", (!customName && !googleAddress) ? "bg-gray-300 cursor-not-allowed" : "bg-black text-white")}>確認新增</button>
            </form>
          </motion.div>
        </>
      )}
