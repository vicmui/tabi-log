...
export default function AddActivityModal({ isOpen, onClose, onSubmit }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 🔥 整合備註
    const finalNote = note || (googleAddress ? `📍 ${googleAddress}` : "");
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed inset-0 m-auto w-full max-w-md h-fit bg-white z-[101] shadow-2xl p-8 rounded-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                 ...
                 <div className="flex items-center gap-2 mt-2"><span className={clsx("w-2 h-2 rounded-full", lat && lng ? "bg-green-500" : "bg-red-500")}/> <span className="text-[10px] text-gray-500">{lat && lng ? `座標已鎖定` : "未有座標"}</span></div>
              </div>
