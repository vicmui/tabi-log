...
const ItemContent = ({ activity, onActivityClick, isReadOnly, config, index, tripId, dayIndex }: any) => {
    ...
    const handleNavigate = (e: React.MouseEvent) => {
        ...
        const dest = hasCoords ? `${activity.lat},${activity.lng}` : encodeURIComponent(activity.address || activity.location);
    ...
    // 🔥 徹底根治 $0：強制轉 Number，並確保 > 0
    const costValue = activity.cost ? Number(activity.cost) : 0;
    const showCost = !isNaN(costValue) && costValue > 0;
    ...
    <div className="relative">
        {!isReadOnly ? (
                                   ...
                                   {showTravelStats && (
                                       <div className="pl-4">
                                          <TravelStats 
                                            ...
                                            dest={{ lat: Number(nextActivity.lat), lng: Number(nextActivity.lng) }} 
                                          />
                                       </div>
                                   )}
                               </motion.div>
            ...
            <div className="space-y-0 pl-2">
                ...
                {activities.map((activity, index) => {
                   const nextActivity = activities[index + 1];
                   const showTravelStats = nextActivity && activity.lat !== undefined && activity.lng !== undefined && nextActivity.lat !== undefined && nextActivity.lng !== undefined;
                   ...
                   return (
                       <div key={activity.id} className="relative mb-0">
                           ...
                           {showTravelStats && (
                               <div className="pl-4">
                                   <TravelStats origin={{ lat: Number(activity.lat), lng: Number(activity.lng) }} dest={{ lat: Number(nextActivity.lat), lng: Number(nextActivity.lng) }} />
                               </div>
                           )}
                       </div>
                   )
