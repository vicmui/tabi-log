...
export default function ActivityDetailModal({ tripId, dayIndex, activityId, onClose }: any) {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      if (photos.length >= 3) { alert("最多只能上傳 3 張相片！"); return; }
                        ...
                        <div className="bg-white border p-1 rounded">
                           <GooglePlacesAutocomplete apiKey={apiKey} selectProps={{ 
                               ...
                               onChange: async (val: any) => { 
                                   if (!val) return; setEditAddress(val.label); 
                                   try { const results = await geocodeByPlaceId(val.value.place_id); const { lat, lng } = await getLatLng(results[0]); setEditLat(lat); setEditLng(lng); if(!editLocation) setEditLocation(val.label.split(',')[0]); } catch(e) { alert("無法獲取座標"); }
                               }, 
               ...
               <>
                 ...
                 <div className="mb-4">
                   ...
                   <div className="flex gap-2 flex-wrap">
                      ...
                      {photos.length < 3 && <label className="w-20 h-20 border border-dashed rounded-lg flex items-center justify-center text-gray-400 cursor-pointer hover:bg-gray-50 hover:border-black hover:text-black transition-colors"><Camera size={20}/><input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} disabled={isUploading}/></label>}
                   </div>
                 </div>
               </>
           )}
