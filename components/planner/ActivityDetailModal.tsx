...
export default function ActivityDetailModal({ tripId, dayIndex, activityId, onClose }: any) {
  ...
  const handleSave = () => {
    if (isEditing) {
      let finalCost = editCost === "" ? 0 : Number(editCost);
      if (currency === "HKD" && finalCost > 0) finalCost = Math.round(finalCost / rate);
    ...
    } else { updateActivity(tripId, dayIndex, activityId, { comment, rating, photos }); onClose(); }
  };
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const file = e.target.files && e.target.files[0]; if (!file || !trip) return;
      try { const filePath = `public/${trip.id}/activities/${uuidv4()}-${file.name}`; const { error } = await supabase.storage.from('trip_files').upload(filePath, file); if (error) throw error; const { data: { publicUrl } } = supabase.storage.from('trip_files').getPublicUrl(filePath); const newPhotos = [publicUrl, ...photos]; setPhotos(newPhotos); updateActivity(tripId, dayIndex, activityId, { photos: newPhotos }); } catch(e: any) { alert("上傳失敗: " + e.message); }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white w-full max-w-lg relative z-10 shadow-2xl overflow-hidden rounded-xl max-h-[90vh] flex flex-col">
        ...
        <div className="p-8 overflow-y-auto">
           {isEditing ? (
               <div className="space-y-5">
                  ...
                  <div className="bg-blue-50 p-3 rounded-lg border border-blue-100"><label className="text-[10px] text-blue-500 font-bold mb-1 block uppercase tracking-widest">連結 Google Map</label>{apiKey ? (<div className="bg-white border p-1 rounded"><GooglePlacesAutocomplete apiKey={apiKey} selectProps={{ placeholder: "輸入地點獲取座標...", onChange: async (val: any) => { if (!val) return; setEditAddress(val.label); try { const results = await geocodeByPlaceId(val.value.place_id); const { lat, lng } = await getLatLng(results[0]); setEditLat(lat); setEditLng(lng); if(!editLocation) setEditLocation(val.label.split(',')[0]); } catch(e) { alert("無法獲取座標"); } }, styles: { control: (p) => ({ ...p, border: 'none', boxShadow: 'none', minHeight: '30px', fontSize: '13px' }) } }} /></div>) : <p className="text-xs text-red-500">API Key Missing</p>}<div className="flex items-center gap-2 mt-2"><span className={clsx("w-2 h-2 rounded-full", editLat && editLng ? "bg-green-500" : "bg-red-500")}/> <span className="text-[10px] text-gray-500">{editLat && editLng ? `座標鎖定` : "未有座標"}</span></div></div>
               ...
               <>
                 ...
                 {/* 🔥 費用顯示 */}
                 {Number(activity.cost) > 0 && <div className="mb-6"><span className="text-xl font-bold font-mono">¥{activity.cost.toLocaleString()}</span></div>}
        ...
        <div className="p-6 border-t border-gray-100 flex gap-2 shrink-0 bg-white"><button onClick={handleDelete} className="text-gray-400 p-3 rounded-lg hover:bg-red-50 hover:text-red-500 transition-colors"><Trash2 size={18}/></button><button onClick={handleSave} className="flex-1 bg-[#333333] text-white py-3 text-xs font-bold tracking-[0.2em] uppercase rounded-lg hover:bg-black transition-colors shadow-lg">{isEditing ? "儲存變更" : "儲存紀錄"}</button></div>
      </motion.div>
      <AnimatePresence>{expandedImg && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setExpandedImg(null)} className="fixed inset-0 z-[200] bg-black/90 flex items-center justify-center p-4"><img src={expandedImg} className="max-w-full max-h-full rounded-lg shadow-2xl" /><button className="absolute top-4 right-4 text-white/80 hover:text-white"><X size={32}/></button></motion.div>)}</AnimatePresence>
    </div>
  );
}