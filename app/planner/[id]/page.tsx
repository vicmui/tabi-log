    ...
    <div className="flex min-h-screen bg-white font-sans text-jp-charcoal overflow-hidden">
      ...
      <main className="flex-1 ml-0 md:ml-64 flex flex-col h-[100dvh]"> {/* 使用 100dvh 確保手機全屏 */}
           ...
           <div className="flex overflow-x-auto snap-x hide-scrollbar py-3 px-4 gap-3">
              {trip.dailyItinerary.map((dayItem, index) => (
                 <button 
                   ...
                   className={clsx(
                     "flex-shrink-0 snap-start flex flex-col items-center justify-center w-16 h-16 rounded-xl border transition-all duration-200",
                     activeDay === index ? "bg-jp-charcoal text-white border-black shadow-md scale-105" : "bg-gray-50 text-gray-400 border-gray-100"
                   )}
                 >
        ...
        <div className="flex-1 relative overflow-y-auto bg-white scroll-smooth md:pl-64"> 
          ...
          <div className="px-4 md:px-12 py-4 md:py-8 max-w-4xl mx-auto min-h-[500px] pb-24">
            ...
            <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-4 sticky top-0 bg-white/95 backdrop-blur z-10 pt-2">
               ...
               <div className="flex gap-2 w-full md:w-auto">
                  <button onClick={handleOpenDayRoute} className="flex-1 md:flex-none justify-center flex items-center gap-2 text-[10px] tracking-widest border border-gray-200 text-gray-500 px-4 py-3 md:py-2 rounded-lg md:rounded-none hover:border-black hover:text-black transition-colors bg-white uppercase">
                    <MapIcon size={12} /> 路線
                  </button>
                  <button onClick={() => setIsModalOpen(true)} className="flex-1 md:flex-none justify-center flex items-center gap-2 text-[10px] tracking-widest bg-jp-charcoal text-white px-5 py-3 md:py-2 rounded-lg md:rounded-none hover:bg-black transition-colors shadow-lg active:scale-95 uppercase">
                    <Plus size={12} /> 新增
                  </button>
               </div>
            </div>
          ...
          {selectedActivityId && (
            <ActivityDetailModal tripId={trip.id} dayIndex={activeDay} activityId={selectedActivityId} onClose={() => setSelectedActivityId(null)} />
          )}
        </div>
      </main>
    </div>
