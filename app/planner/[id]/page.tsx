...
import { ArrowLeft, Plus, MapPin, Clock, Map as MapIcon, List as ListIcon, CalendarX, Camera, Navigation, Share, Globe, Sun, Cloud, CloudSun, CloudRain, Snowflake, Edit } from "lucide-react";
...
function getTripCoords(title: string, destLat?: number, destLng?: number): { lat: number; lng: number } {
...
export default function PlannerPage() {
  ...
  useEffect(() => {
    ...
    // Always pre-fill dummy first so UI shows something immediately
    const dummy: Record<string, { temp: string; code: number }> = {};
    trip.dailyItinerary.forEach(d => { dummy[d.date] = { temp: "15/25", code: 1 }; });
    setWeatherMap(dummy);

    // Too far in future (>15d) or too far in past — stay with dummy
    if (daysUntilTrip > 15 || daysUntilTrip < -trip.dailyItinerary.length) return;

    // Use real destination coords
    const { lat, lng } = getTripCoords(trip.title, trip.destLat, trip.destLng);

    // Clamp end date to Open-Meteo's 15-day forecast limit
    const maxDate = new Date();
    maxDate.setDate(maxDate.getDate() + 15);
    const clampedEnd = parseISO(endDate) > maxDate
      ? maxDate.toISOString().split("T")[0]
      : endDate;

    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=auto&start_date=${startDate}&end_date=${clampedEnd}`;
  ...
  const displayLocation = currentDailyItinerary?.customLocation
    || (currentDailyItinerary?.activities?.length > 0 ? currentDailyItinerary.activities[0].location.split(" ")[0] : "自由探索");
  ...
  const handleOpenDayRoute = () => {
    ...
    const acts = currentDailyItinerary.activities.filter(a => a && (a.address || a.location));
            ...
            {trip.dailyItinerary.map((dayItem, index) => {
              ...
              return (
                <button key={dayItem.day} onClick={() => setActiveDay(index)}
                  className={`w-full text-left py-6 px-8 transition-all duration-300 group relative border-b border-gray-50 last:border-0 ${activeDay === index ? "bg-gray-50" : "hover:bg-gray-50"}`}>
            ...
            <div className="absolute bottom-0 left-0 right-0 px-6 md:px-16 pb-6 pt-20">
              <div className="animate-fade-in-up">
                ...
                <button onClick={handleEditLocation} className="flex items-center gap-3 text-[10px] text-gray-600 tracking-[0.3em] uppercase font-bold mt-2 bg-white/90 backdrop-blur-sm w-fit px-3 py-1 rounded-full shadow-md hover:bg-white transition-all z-20">
                  ...
                  <span className="w-px h-3 bg-gray-300" /><Clock size={10} /><span>{currentDailyItinerary?.date}</span>
                </button>
              </div>
            </div>
            ...
            <div className="w-full">
              {viewMode === "list" ? (
                ...
                <div className="h-[65dvh] w-full border border-gray-200 rounded-3xl overflow-hidden shadow-sm bg-gray-50">
                  {/* key forces full remount on day change — fixes first-day load bug */}
                  <TripMap
