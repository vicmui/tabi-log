  ...
  return (
    <div className="flex h-screen bg-white font-sans text-jp-black overflow-hidden">
      ...
      <main className="flex-1 flex flex-col md:flex-row h-full ml-0 md:ml-64 relative overflow-hidden">
        <div className="md:hidden flex items-center justify-between p-4 border-b border-gray-100 bg-white shrink-0 z-40"><Link href="/" className="text-gray-400"><ArrowLeft size={22}/></Link><h1 className="font-bold text-sm tracking-widest uppercase truncate px-4">{trip.title}</h1><button onClick={() => setIsModalOpen(true)} className="bg-black text-white p-2 rounded-lg shadow-sm"><Plus size={20}/></button></div>
                 ...
                 return (
                   <button key={dayItem.day} onClick={() => setActiveDay(index)} className={clsx(
                     ...
                     activeDay === index ? "bg-black text-white border-black shadow-lg scale-105" : "bg-white text-gray-400 border-gray-100"
                      ...
                      <span className="text-[9px] font-bold uppercase tracking-widest">{format(dateObj, 'EEE')}</span>
                      <span className="text-xl font-bold leading-none my-1">{format(dateObj, 'd')}</span>
                      <span className="text-[8px] font-medium opacity-60 uppercase">{format(dateObj, 'MMM')}</span>
                   </button>
                 )
              })}
          ...
          <div className="px-4 md:px-16 py-8 max-w-5xl mx-auto min-h-[500px] pb-32">
            <div className="flex justify-between items-center mb-10 border-b border-gray-100 pb-4 sticky top-0 bg-white/95 backdrop-blur z-10 pt-2">
               ...
               <div className="flex gap-2 w-full md:w-auto overflow-x-auto no-scrollbar justify-end">
                  <button onClick={() => setViewMode(viewMode === 'list' ? 'map' : 'list')} className="flex-none flex items-center gap-2 text-[10px] tracking-widest border border-gray-200 text-gray-500 px-4 py-2 hover:border-black transition-colors bg-white uppercase rounded-lg font-bold">
                    {viewMode === 'list' ? <><MapIcon size={12} /> Map</> : <><ListIcon size={12} /> List</>}
                  </button>
                  <button onClick={() => setIsModalOpen(true)} className="flex-none flex items-center gap-2 text-[10px] tracking-widest bg-black text-white px-5 py-2 hover:bg-gray-800 transition-colors shadow-lg uppercase rounded-lg font-bold"><Plus size={12} /> Add</button>
               </div>
            </div>
            {viewMode === 'list' ? (
