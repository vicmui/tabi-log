      ...
      <main className="flex-1 flex flex-col md:flex-row h-full ml-0 md:ml-64 relative">
          ...
          <div className="flex-1">
            {trip.dailyItinerary.map((dayItem, index) => (
              <button key={dayItem.day} onClick={() => setActiveDay(index)} className={`w-full text-left p-6 transition-all duration-300 group relative border-b border-gray-50 ${activeDay === index ? "bg-jp-charcoal text-white" : "bg-white hover:bg-gray-50 text-gray-400 hover:text-jp-charcoal"}`}>
                <div className="flex justify-between items-baseline relative z-10"><span className="text-xs font-bold tracking-[0.2em] uppercase">Day {dayItem.day}</span><span className="text-[10px] opacity-60 font-medium">{dayItem.weather}</span></div>
            ...
            <button onClick={() => addDayToTrip(trip.id)} className="w-full py-4 text-xs text-gray-400 hover:text-black hover:bg-gray-50 border-t border-gray-100 flex items-center justify-center gap-2 uppercase tracking-widest"><Plus size={14}/> 加一天</button>
          </div>
        </div>
        ...
        <div className="md:hidden w-full bg-white border-b border-gray-100 z-20 shadow-sm shrink-0">
           <div className="flex overflow-x-auto snap-x hide-scrollbar py-3 px-4 gap-3 items-center">
              ...
              <button onClick={() => addDayToTrip(trip.id)} className="flex-shrink-0 flex flex-col items-center justify-center w-12 h-16 rounded-xl border border-dashed border-gray-300 text-gray-400 hover:bg-gray-50"><Plus size={20}/></button>
           </div>
        </div>
          ...
          <div className="px-4 md:px-12 py-4 md:py-8 max-w-4xl mx-auto min-h-[500px] pb-24">
            <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-4 sticky top-0 bg-white/95 backdrop-blur z-10 pt-2">
               ...
               <div className="flex gap-2 w-full md:w-auto overflow-x-auto no-scrollbar justify-end">
                  ...
                  <button onClick={() => setIsModalOpen(true)} className="flex-none flex items-center gap-2 text-[10px] tracking-widest bg-jp-charcoal text-white px-4 py-2 rounded-lg hover:bg-black transition-colors shadow-lg active:scale-95 uppercase"><Plus size={12} /> 新增</button>
               </div>
            </div>
