...
function getTripCoords(title: string, destLat?: number, destLng?: number): { lat: number; lng: number } {
  // If trip has explicit destination coords, use them first
  ...
  if (s.includes('香港') || s.includes('hong kong')) return CITY_COORDS.hongkong
  ...
  return CITY_COORDS.osaka // last-resort fallback
}
...
export default function PlannerPage() {
  ...
  // ── Weather API: use trip's actual destination coords ────────────────────
  useEffect(() => {
    const fetchWeather = async () => {
      ...
      // Outside 15-day forecast window → use dummy data
      if (daysUntilTrip > 15 || daysUntilTrip < -trip.dailyItinerary.length) {
        const dummyMap: Record<string, { temp: string; code: number }> = {};
        trip.dailyItinerary.forEach(day => {
          dummyMap[day.date] = { temp: "15°/25°", code: 1 };
        });
        setWeatherMap(dummyMap);
        return;
      }

      // ✅ Use trip's real destination coords (not hardcoded Osaka)
      const { lat, lng } = getTripCoords(trip.title, trip.destLat, trip.destLng);

      try {
        const res = await fetch(
          `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=auto&start_date=${startDate}&end_date=${endDate}`
        );
        ...
        // Silently fall back — dummy data already set or weatherMap stays empty
      }
    };
    fetchWeather();
  }, [trip?.id]); // Re-fetch only when trip changes, not on every render
            ...
            <div className="w-full">
              {viewMode === 'list' ? (
                ...
                <div className="h-[65dvh] w-full border border-gray-200 rounded-3xl overflow-hidden shadow-sm bg-gray-50">
                  {/*
                    ✅ KEY FIX: key={`${trip.id}-${activeDay}`} forces TripMap to fully remount
                    whenever the selected day changes, eliminating the "first day doesn't load"
                    stale-state bug caused by the map not re-initialising its center/markers.
                  */}
                  <TripMap
