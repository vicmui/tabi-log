...
export default function PlannerPage() {
  ...
  useEffect(() => {
    if (trip) { setEditTitle(trip.title); setEditStartDate(trip.startDate); }
  }, [trip]);

  useEffect(() => {
    if (trip && activeDay >= trip.dailyItinerary.length) { setActiveDay(Math.max(0, trip.dailyItinerary.length - 1)); }
  }, [trip, activeDay]);

  if (!isMounted) return null;
  if (!trip) return <div className="p-10 text-center text-xs tracking-widest text-gray-400">LOADING...</div>;
  ...
  const handleDeleteDay = () => { if (trip.dailyItinerary.length <= 1) { alert("最少要保留一天行程！"); return; } if (confirm(`確定要刪除 Day ${activeDay + 1} 及其行程嗎？`)) { deleteDayFromTrip(trip.id, activeDay); } };
    ...
    <div className="flex h-screen bg-white font-sans text-jp-black overflow-hidden">
      ...
      <main className="flex-1 flex flex-col md:flex-row h-full ml-0 md:ml-64 relative">
        <div className="md:hidden flex items-center justify-between p-4 border-b border-gray-100 bg-white shrink-0 z-30">
           ...
           <button onClick={()=>setIsSettingsOpen(true)} className="text-sm font-medium tracking-wide truncate w-2/3 text-center flex items-center justify-center gap-2">
              {trip.title} <Settings size={12} className="text-gray-300"/>
           </button>
           <div className="w-5" /> 
        </div>
        ...
        <div className="hidden md:flex w-64 border-r border-gray-100 bg-white h-full overflow-y-auto flex-col shrink-0 z-20 pt-10">
          <div className="px-8 pb-8 border-b border-gray-50 sticky top-0 bg-white z-10">
            ...
            <div className="group cursor-pointer" onClick={()=>setIsSettingsOpen(true)}>
               <h2 className="text-lg font-light leading-snug mb-2 text-black tracking-wide group-hover:underline decoration-1 underline-offset-4">{trip.title}</h2>
               <div className="flex items-center gap-2 text-[9px] text-gray-400 tracking-[0.2em] uppercase font-light"><Calendar size={10} /><span>{trip.startDate}</span></div>
            </div>
          </div>
            ...
            {trip.dailyItinerary.map((dayItem, index) => (
              <button key={dayItem.day} onClick={() => setActiveDay(index)} className={`w-full text-left py-4 px-8 transition-all duration-300 group relative ${activeDay === index ? "bg-gray-50" : "hover:bg-gray-50"}`}>
                <div className="flex justify-between items-center relative z-10">
                   ...
                   <span className="text-[9px] text-gray-300 font-light">{dayItem.weather}</span>
                </div>
        ...
        <div className="md:hidden w-full bg-white border-b border-gray-100 z-20 shadow-sm shrink-0">
           <div className="flex overflow-x-auto snap-x no-scrollbar py-3 px-4 gap-3 items-center">
              {trip.dailyItinerary.map((dayItem, index) => (
                 <button key={dayItem.day} onClick={() => setActiveDay(index)} className={clsx("flex-shrink-0 snap-start flex flex-col items-center justify-center w-14 h-14 border transition-all duration-200", activeDay === index ? "bg-black text-white border-black" : "bg-white text-gray-400 border-gray-200")}>
                    <span className="text-[9px] font-light uppercase tracking-wider">Day</span>
                    <span className="text-lg font-light leading-none">{dayItem.day}</span>
                 </button>
              ))}
        ...
        <div className="flex-1 relative overflow-y-auto bg-white scroll-smooth h-full"> 
          <div className="h-40 md:h-72 relative w-full shrink-0">
            ...
            <div className="absolute bottom-0 left-0 right-0 px-6 md:px-16 pb-6 pt-20">
               <div className="animate-fade-in-up">
                 <h3 className="text-4xl md:text-6xl font-light tracking-[0.2em] text-black mb-2 uppercase" style={{fontFamily: 'var(--font-inter)'}}>Day {activeDay + 1}</h3>
                 <div className="flex items-center gap-3 text-[10px] text-gray-500 tracking-[0.3em] uppercase font-light">
                    <MapPin size={10} /><span>Osaka</span><span className="w-8 h-[1px] bg-gray-300"></span><Clock size={10} /><span>{currentDailyItinerary?.date}</span>
                 </div>
               </div>
            </div>
          </div>
          ...
          <div className="px-4 md:px-16 py-8 max-w-5xl mx-auto min-h-[500px] pb-32">
            <div className="flex justify-between items-center mb-10 border-b border-gray-100 pb-4 sticky top-0 bg-white/95 backdrop-blur z-10 pt-2">
               <div className="flex items-center gap-4">
                  ...
                  <button onClick={handleDeleteDay} className="text-gray-300 hover:text-red-400 transition-colors" title="Delete"><CalendarX size={14} /></button>
               </div>
            ...
            ) : (<div className="text-center py-32 text-gray-300 text-[10px] tracking-[0.3em] uppercase font-light">No Activities</div>)}
          </div>
          ...
          {isSettingsOpen && (
             <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/80 backdrop-blur-sm">
                <div className="bg-white p-10 w-full max-w-md shadow-2xl relative border border-gray-100">
                   <h2 className="font-light text-2xl mb-8 tracking-widest text-center uppercase">Settings</h2>
                   <div className="space-y-6">
                      <div><label className="text-[10px] text-gray-400 block mb-2 tracking-widest uppercase">Trip Title</label><input className="w-full border-b border-gray-200 p-2 text-sm font-light focus:outline-none focus:border-black transition-colors" value={editTitle} onChange={e=>setEditTitle(e.target.value)}/></div>
                      <div>
                          <label className="text-[10px] text-gray-400 block mb-2 tracking-widest uppercase">Start Date</label>
                          <input type="date" className="w-full border-b border-gray-200 p-2 text-sm font-light focus:outline-none focus:border-black transition-colors" value={editStartDate} onChange={e=>setEditStartDate(e.target.value)}/>
                      </div>
                   </div>
                   <div className="flex gap-4 mt-10">
                      <button onClick={()=>setIsSettingsOpen(false)} className="flex-1 border border-gray-200 py-3 text-[10px] tracking-widest uppercase hover:bg-gray-50 transition-colors">Cancel</button>
                      <button onClick={handleSaveSettings} className="flex-1 bg-black text-white py-3 text-[10px] tracking-widest uppercase hover:opacity-80 transition-opacity">Save</button>
                   </div>
                </div>
             </div>
          )}
