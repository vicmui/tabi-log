...
import { ArrowLeft, Plus, MapPin, Clock, Map as MapIcon, List as ListIcon, CalendarX, Camera, Navigation, Share, Globe, Sun, Cloud, CloudSun, CloudRain, Snowflake, Edit, Move, Check, X, ImagePlus } from "lucide-react";
...
// ── Fullscreen drag-to-reposition overlay ────────────────────────────────────
function RepositionOverlay({ src, initialY, onConfirm, onCancel }: {
  src: string; initialY: number;
  onConfirm: (y: number) => void; onCancel: () => void;
}) {
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    setPosY(clamp(startPosY.current - (delta / h) * 100));
  };
  const onPointerUp = () => { dragging.current = false; };

  return (
    <div className="fixed inset-0 z-[300] bg-black flex flex-col select-none">
      <div className="absolute top-0 inset-x-0 z-10 bg-black/70 backdrop-blur-sm text-white text-center py-3 px-4">
        <p className="text-xs font-bold tracking-widest uppercase flex items-center justify-center gap-2">
          <Move size={14} /> 上下拖動調整封面焦點位置
        </p>
      </div>
      ...
      >
        ...
        <img
          src={src} alt="reposition" draggable={false}
        ...
        <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 pointer-events-none flex flex-col items-center gap-1">
          ...
          <span className="bg-black/50 text-white text-[10px] px-3 py-0.5 rounded-full tracking-widest">焦點區域</span>
        </div>
      </div>

      <div className="bg-black/80 backdrop-blur-sm flex items-center justify-between px-6 py-4 gap-4 shrink-0">
        <button onClick={onCancel}
          className="flex items-center gap-2 text-white/70 hover:text-white text-xs uppercase tracking-widest px-4 py-2 border border-white/20 hover:border-white transition-colors">
          <X size={14} /> 取消
        </button>
        <p className="text-white/40 text-[10px] text-center flex-1 leading-relaxed hidden sm:block">
          建議尺寸 <span className="text-white/60 font-bold">2400 × 800px</span> · 橫向構圖 · JPG / PNG
        </p>
        <button onClick={() => onConfirm(posY)}
          className="flex items-center gap-2 bg-white text-black text-xs font-bold uppercase tracking-widest px-6 py-2 hover:bg-gray-100 transition-colors">
          <Check size={14} /> 完成
        </button>
      </div>
    </div>
...
export default function PlannerPage() {
  ...
  // Cover position map: key = `${tripId}-${dayIndex}`, value = objectPosition Y% (0-100)
  const [positionMap, setPositionMap] = useState<Record<string, number>>({});
  const [repositionState, setRepositionState] = useState<{ src: string; dayIndex: number } | null>(null);
  ...
  // Upload → save URL → open reposition overlay automatically
  const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!error) {
      ...
      setRepositionState({ src: publicUrl, dayIndex: activeDay });
    }
    e.target.value = "";
  };

  const handleRepositionConfirm = (y: number) => {
    if (!repositionState) return;
    savePosMap({ ...positionMap, [`${trip.id}-${repositionState.dayIndex}`]: y }, trip.id);
  ...
  return (
    <>
      {repositionState && (
        <RepositionOverlay
          src={repositionState.src}
          initialY={getCoverPos(trip.id, repositionState.dayIndex)}
          ...
          <div className="flex-1 relative overflow-y-auto bg-white scroll-smooth h-full no-scrollbar pb-32">
            ...
            <div className="h-44 md:h-80 relative w-full shrink-0 group">
              {coverSrc ? (
                ...
                /* ── Prominent empty state placeholder ── */
                <label className="w-full h-full bg-gradient-to-br from-gray-100 to-gray-200 flex flex-col items-center justify-center gap-3 cursor-pointer hover:from-gray-200 hover:to-gray-300 transition-colors">
                  ...
                  <div className="text-center px-4">
                    ...
                    <p className="text-xs text-gray-400 mt-1.5">
                      建議尺寸 <span className="font-bold text-gray-600">2400 × 800px</span>，橫向構圖，JPG / PNG
                    </p>
                    <p className="text-[11px] text-gray-300 mt-1">上傳後可即時拖動調整焦點位置</p>
                  </div>
                </label>
              )}
              ...
              {/* Hover controls — only when cover exists */}
              {coverSrc && (
                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                  {/* Reposition */}
                  <button
                    onClick={() => setRepositionState({ src: coverSrc, dayIndex: activeDay })}
                  ...
                  {/* Upload new */}
                  <label
                    className="flex items-center gap-1.5 bg-white/85 hover:bg-white text-black text-[10px] font-bold uppercase tracking-widest px-3 py-2 backdrop-blur-sm shadow-sm cursor-pointer transition-all"
              ...
              {/* Day label when no cover */}
              {!coverSrc && (
                <div className="absolute bottom-4 left-6 md:left-16 pointer-events-none">
                  <h3 className="text-4xl md:text-7xl font-bold tracking-tight uppercase leading-none text-gray-300">Day {activeDay + 1}</h3>
                </div>
              )}
            </div>
