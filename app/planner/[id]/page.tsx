    ...
    <div className="flex h-screen bg-white font-sans text-jp-black overflow-hidden">
      ...
      <main className="flex-1 flex flex-col md:flex-row h-full ml-0 md:ml-64 relative">
        <div className="md:hidden flex items-center justify-between p-4 border-b border-gray-100 bg-white shrink-0 z-30">
           ...
           <button onClick={()=>setIsSettingsOpen(true)} className="text-sm font-medium tracking-wide truncate w-2/3 text-center flex items-center justify-center gap-2">{trip.title} <Settings size={12} className="text-gray-300"/></button>
           <div className="w-5" />
        </div>
        ...
        <div className="hidden md:flex w-64 border-r border-gray-100 bg-white h-full overflow-y-auto flex-col shrink-0 z-20 pt-10">
          <div className="px-8 pb-8 border-b border-gray-50 sticky top-0 bg-white z-10">
            ...
            <div className="group cursor-pointer" onClick={()=>setIsSettingsOpen(true)}><h2 className="text-lg font-light leading-snug mb-2 text-black tracking-wide group-hover:underline">{trip.title}</h2><div className="flex items-center gap-2 text-[9px] text-gray-400 tracking-[0.2em] uppercase"><Calendar size={10} /><span>{trip.startDate}</span></div></div>
          </div>
          <div className="flex-1 py-4">{trip.dailyItinerary.map((dayItem, index) => (<button key={dayItem.day} onClick={() => setActiveDay(index)} className={`w-full text-left py-4 px-8 transition-all duration-300 group relative ${activeDay === index ? "bg-gray-50" : "hover:bg-gray-50"}`}><div className="flex justify-between items-center relative z-10"><span className={clsx("text-xs tracking-[0.2em] uppercase", activeDay === index ? "font-medium text-black" : "font-light text-gray-400")}>Day {dayItem.day}</span><span className="text-[9px] text-gray-300">{dayItem.weather}</span></div><div className="text-[9px] mt-1 text-gray-300">{dayItem.date}</div>{activeDay === index && <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-black" />}</button>))}
            <button onClick={() => addDayToTrip(trip.id)} className="w-full py-6 text-[10px] text-gray-300 hover:text-black flex items-center justify-center gap-2 uppercase tracking-[0.2em]"><Plus size={12}/> Add Day</button>
          </div>
        </div>
        ...
        <div className="md:hidden w-full bg-white border-b border-gray-100 z-20 shadow-sm shrink-0">
           <div className="flex overflow-x-auto snap-x no-scrollbar py-3 px-4 gap-3 items-center">
              {trip.dailyItinerary.map((dayItem, index) => (<button key={dayItem.day} onClick={() => setActiveDay(index)} className={clsx("flex-shrink-0 snap-start flex flex-col items-center justify-center w-14 h-14 border rounded-xl", activeDay === index ? "bg-black text-white" : "bg-white text-gray-400 border-gray-200")}><span className="text-[9px] uppercase">Day</span><span className="text-lg font-light">{dayItem.day}</span></button>))}
              <button onClick={() => addDayToTrip(trip.id)} className="flex-shrink-0 w-10 h-14 border border-dashed text-gray-400 flex items-center justify-center"><Plus size={16}/></button>
           </div>
        </div>
        ...
        <div className="flex-1 relative overflow-y-auto bg-white scroll-smooth h-full"> 
          <div className="h-40 md:h-72 relative w-full shrink-0 group">
            ...
            <div className="absolute inset-0 bg-black/10" /><div className="absolute inset-0 bg-gradient-to-t from-white via-transparent" />
            <div className="absolute bottom-0 left-0 right-0 px-6 md:px-16 pb-6 pt-20">
               <div><h3 className="text-4xl md:text-6xl font-light tracking-[0.2em] text-black mb-2 uppercase" style={{fontFamily: 'var(--font-inter)'}}>Day {activeDay + 1}</h3><div className="flex items-center gap-3 text-[10px] text-gray-500 tracking-[0.3em] uppercase"><MapPin size={10} /><span>大阪</span><span className="w-8 h-[1px] bg-gray-300"></span><Clock size={10} /><span>{currentDailyItinerary?.date}</span></div></div>
            </div>
          </div>
          ...
          <div className="px-4 md:px-16 py-8 max-w-5xl mx-auto min-h-[500px] pb-32">
            <div className="flex justify-between items-center mb-10 border-b border-gray-100 pb-4 sticky top-0 bg-white/95 backdrop-blur z-10 pt-2">
               <div className="flex items-center gap-4"><span className="text-[10px] font-bold tracking-[0.2em] text-black uppercase">行程</span><button onClick={handleDeleteDay} className="text-gray-300 hover:text-red-400" title="Delete"><CalendarX size={14} /></button></div>
               <div className="flex gap-3 w-full md:w-auto overflow-x-auto no-scrollbar justify-end">
                  {/* 🔥 將「路線」掣改為 Link，並移除「分享」掣 */}
                  <Link href={`/planner/${trip.id}/map`} className="flex-none flex items-center gap-2 text-[10px] tracking-widest border border-gray-200 text-gray-500 px-4 py-2 hover:border-black hover:text-black transition-colors bg-white uppercase rounded-lg">
                    <MapIcon size={12} /> 地圖總覽
                  </Link>
