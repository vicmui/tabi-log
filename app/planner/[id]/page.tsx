...
export default function PlannerPage() {
  ...
  const handleOpenDayRoute = () => {
    ...
    const acts = currentDailyItinerary.activities.filter(a => (a.lat && a.lng) || a.address || a.location);
    ...
    <div className="flex h-screen bg-white font-sans text-[#333333] overflow-hidden">
      ...
      <main className="flex-1 flex flex-col md:flex-row h-full ml-0 md:ml-64 relative overflow-hidden">
        ...
        <div className="md:hidden flex items-center justify-between p-4 border-b border-gray-100 bg-white shrink-0 z-40">
           ...
           <button onClick={() => setIsModalOpen(true)} className="bg-black text-white p-2 rounded-lg shadow-sm active:scale-95 transition-transform">
             <Plus size={20}/>
           </button>
        </div>
        ...
        <div className="md:hidden w-full bg-white border-b border-gray-100 z-30 shrink-0 shadow-sm overflow-hidden">
           <div className="flex overflow-x-auto snap-x snap-mandatory no-scrollbar py-4 px-4 gap-3 items-center">
              {trip.dailyItinerary.map((dayItem, index) => {
                 ...
                 return (
                   <button key={dayItem.day} onClick={() => setActiveDay(index)} className={clsx(
                     "flex-shrink-0 snap-center flex flex-col items-center justify-center w-24 h-28 border transition-all duration-300 rounded-2xl", 
                     activeDay === index ? "bg-black text-white border-black shadow-lg scale-105" : "bg-white text-gray-400 border-gray-200"
                      ...
                      <span className="text-2xl font-semibold leading-none my-1">{dayItem.day}</span>
                      <span className="text-[9px] font-medium opacity-60 mb-2">{format(parseISO(dayItem.date), 'MM/dd')}</span>
                      <div className="flex flex-col items-center gap-1 border-t border-current/10 pt-2 w-full">
                         <WeatherIcon code={info?.code} />
              ...
              <button onClick={() => addDayToTrip(trip.id)} className="flex-shrink-0 flex items-center justify-center w-12 h-28 border border-dashed border-gray-200 text-gray-300 rounded-2xl snap-center"><Plus size={24}/></button>
           </div>
        </div>
        ...
        <div className="flex-1 relative overflow-y-auto bg-white scroll-smooth h-full no-scrollbar pb-32"> 
          ...
          <div className="h-44 md:h-80 relative w-full shrink-0 group">
            ...
            <label className="absolute top-4 right-4 bg-white/50 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity z-20 cursor-pointer text-black hover:bg-white">
                <Camera size={16}/>
          ...
          <div className="px-4 md:px-12 py-6 max-w-5xl mx-auto min-h-[500px]">
            ...
            <div className="mb-8 border-b border-gray-100 pb-4 sticky top-0 bg-white/95 backdrop-blur z-30 pt-2">
               <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex gap-2 w-full md:w-auto overflow-x-auto hide-scrollbar pb-1">
                      <button onClick={handleCopyShareLink} className="flex-1 md:flex-none flex items-center justify-center gap-2 text-[10px] font-bold tracking-widest border border-gray-200 text-gray-500 py-2.5 rounded-xl bg-white uppercase hover:border-black transition-all whitespace-nowrap px-5">
                        <Share size={14} /> 分享
                      </button>
                      <button onClick={handleOpenDayRoute} className="flex-1 md:flex-none flex items-center justify-center gap-2 text-[10px] font-bold tracking-widest border border-gray-200 text-gray-500 py-2.5 rounded-xl bg-white uppercase hover:border-black transition-all whitespace-nowrap px-5">
                        <Navigation size={14} /> 路線
                      </button>
                      <button onClick={() => setViewMode(viewMode === 'list' ? 'map' : 'list')} className="flex-1 md:flex-none flex items-center justify-center gap-2 text-[10px] font-bold tracking-widest border border-gray-200 text-gray-500 py-2.5 rounded-xl bg-white uppercase hover:border-black transition-all whitespace-nowrap px-5">
                        {viewMode === 'list' ? <><MapIcon size={14} /> 地圖</> : <><ListIcon size={14} /> 列表</>}
                      </button>
                      <button onClick={() => setIsModalOpen(true)} className="hidden md:flex flex-none items-center gap-2 text-[10px] tracking-widest bg-black text-white px-6 py-2.5 hover:bg-gray-800 transition-colors shadow-lg uppercase rounded-xl font-bold">
                        <Plus size={14} /> 新增活動
                      </button>
                  </div>
               </div>
            ...
            <div className="w-full">
                {viewMode === 'list' ? (
                    ...
                    <div className="h-[60vh] w-full border border-gray-200 rounded-3xl overflow-hidden shadow-sm bg-gray-50">
