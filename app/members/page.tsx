    ...
    return (
      <div className="flex min-h-screen bg-white">
        ...
        <main className="flex-1 ml-0 md:ml-64 p-12 flex items-center justify-center">
           <div className="text-center text-gray-500 text-xs tracking-widest animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {trip.members.map(member => (
            <div key={member.id} className={clsx("group relative bg-white border p-6 flex flex-col items-center transition-all duration-300 rounded-none", editingMemberId === member.id ? "border-black ring-1 ring-black" : "border-gray-200 hover:shadow-lg")}>
              ...
              <div className="absolute top-4 right-4 flex gap-2">
                 ...
                 <button onClick={() => handleDeleteMember(member.id)} className="text-gray-500 hover:text-red-500 transition-colors p-1"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
          ...
          <div className={clsx("border border-dashed p-6 flex flex-col items-center justify-center gap-4 rounded-none transition-colors", editingMemberId ? "bg-white border-black" : "bg-gray-50 border-gray-300")}>
             ...
             <label className="w-24 h-24 bg-white rounded-full flex items-center justify-center text-gray-500 border border-gray-200 cursor-pointer hover:border-black relative overflow-hidden group transition-all" title="400x400 JPG/PNG max 2MB">
                {avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity"/> : <div className="flex flex-col items-center gap-1"><Camera size={20} /><span className="text-[11px]">上傳</span></div>}
