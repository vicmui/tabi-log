    ...
    <div className="flex min-h-screen bg-white font-sans text-jp-charcoal">
      ...
      <main className="flex-1 ml-0 md:ml-64 p-8 md:p-12 pb-24">
        ...
        <div className="flex gap-8 border-b border-gray-100 mb-8 overflow-x-auto no-scrollbar">
           {['Packing','Todo','Shopping'].map(t => (<button key={t} onClick={()=>setActiveTab(t)} className={`pb-4 text-xs font-bold tracking-[0.2em] uppercase whitespace-nowrap ${activeTab===t?'border-b-2 border-black':'text-gray-300'}`}>{tabNames[t]}</button>))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           {currentItems.map(item => {
               ...
               <div key={item.id} className="p-4 border border-gray-100 bg-white hover:shadow-md transition-shadow relative group rounded-lg">
                  <div className="flex justify-between items-start mb-2">
                     <button onClick={()=>togglePlanItem(trip.id, item.id)} className={item.isCompleted ? "text-gray-300" : "text-black"}>{item.isCompleted ? <CheckCircle2 size={20}/> : <Circle size={20}/>}</button>
                     <span className={`text-[10px] uppercase font-bold ${priorityColor[item.priority]}`}>{item.priority}</span>
                  </div>
                  {item.imageUrl && (<div className="w-full h-32 mb-3 bg-gray-50 rounded overflow-hidden border border-gray-100"><img src={item.imageUrl} className="w-full h-full object-cover" /></div>)}
                  <p className={`font-medium ${item.isCompleted?'line-through text-gray-300':''}`}>{item.text}</p>
                  <div className="mt-2 space-y-1">{item.location && <p className="text-xs text-gray-400">📍 {item.location}</p>}{item.estimatedCost && <p className="text-xs text-gray-400">💰 ¥{item.estimatedCost}</p>}</div>
                  {assigned && (<div className="mt-3 flex items-center gap-2 bg-gray-50 p-1 rounded-full w-fit pr-2 border border-gray-100"><img src={assigned.avatar} className="w-5 h-5 rounded-full"/><span className="text-[10px] text-gray-500">{assigned.name}</span></div>)}
                  <button onClick={() => deletePlanItem(trip.id, item.id)} className="absolute top-4 right-2 opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-500 transition-opacity"><Trash2 size={16} /></button>
               </div>
           )})}
           ...
           <div className="border border-dashed border-gray-300 p-4 bg-gray-50 rounded-lg">
              ...
              {activeTab === 'Shopping' && (<>
                    ...
                    <div className="mb-3">{imageUrl ? (<div className="relative w-full h-24 rounded overflow-hidden group"><img src={imageUrl} className="w-full h-full object-cover" /><button onClick={()=>setImageUrl("")} className="absolute top-1 right-1 bg-black/50 text-white p-1 rounded-full"><X size={12}/></button></div>) : (<label className="flex items-center gap-2 text-xs text-gray-400 cursor-pointer hover:text-black transition-colors">{isUploading ? "上傳中..." : <><ImageIcon size={14}/> 上傳參考圖片</>}<input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} disabled={isUploading} /></label>)}</div>
              </>)}
              ...
              <div className="flex gap-2 mb-4 overflow-x-auto no-scrollbar"><span className="text-[10px] text-gray-400 flex items-center shrink-0">指派:</span>{trip.members.map(m => (<button key={m.id} onClick={()=>setAssignee(m.id === assignee ? "" : m.id)} className={`w-6 h-6 rounded-full border shrink-0 ${assignee===m.id ? 'border-black scale-110 shadow-sm' : 'border-transparent opacity-50'}`}><img src={m.avatar} className="w-full h-full rounded-full"/></button>))}</div>
