        ...
        <div ref={setNodeRef} style={style} className={clsx("p-4 border bg-white hover:shadow-md transition-shadow relative group rounded-none flex items-start gap-4", item.isCompleted && "opacity-50")}>
            <div className="flex-1 flex items-start gap-4">
                ...
                <div className="flex-1">
                    <p className={clsx("font-medium", item.isCompleted && "line-through text-gray-500")}>{item.text}</p>
                    <div className="flex items-center gap-4 mt-2">
                        <span className={`text-[11px] uppercase font-semibold tracking-widest px-2 py-0.5 rounded-sm ${priorityColor[item.priority]}`}>{item.priority}</span>
                    ...
                    {assigned && (
                        <div className="mt-2 flex items-center gap-2 bg-gray-50 p-1 rounded-full w-fit pr-2 border border-gray-100">
                            ...
                            <span className="text-xs text-gray-500">{assigned.name}</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="absolute top-4 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => onEdit(item)} className="p-1 text-gray-500 hover:text-black"><Edit size={14}/></button>
                <button onClick={() => onDeleteRequest(item.id)} className="p-1 text-gray-500 hover:text-red-500"><Trash2 size={14} /></button>
            </div>
            <div {...attributes} {...listeners} className="absolute inset-y-0 right-10 flex items-center px-2 cursor-grab touch-none text-gray-400">
      ...
      <main className="flex-1 min-w-0 ml-0 md:ml-64 p-5 sm:p-8 md:p-12 pb-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           ...
           <div className={clsx("border border-dashed p-6 bg-gray-50 rounded-none h-fit sticky top-10", editingItemId ? "border-black bg-white" : "border-gray-300")}>
              ...
              <div className="flex gap-2 mb-4 overflow-x-auto no-scrollbar"><span className="text-xs text-gray-500 flex items-center shrink-0">指派:</span>{trip.members.map(m => (<button key={m.id} onClick={()=>setAssignee(m.id === assignee ? "" : m.id)} className={`w-6 h-6 rounded-full border shrink-0 overflow-hidden ${assignee===m.id ? 'border-black scale-110' : 'border-transparent opacity-50'}`}><img src={m.avatar} className="w-full h-full object-cover"/></button>))}</div>
