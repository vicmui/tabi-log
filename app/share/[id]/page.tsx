    ...
    <div className="min-h-screen bg-white font-sans text-[#333333] pb-12">
      ...
      <div className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-gray-100 px-6 py-4 flex flex-col gap-2">
          <div className="flex justify-between items-start">
             ...
             <div className="text-xs font-bold bg-black text-white px-3 py-1 rounded-full uppercase tracking-widest shrink-0">Read Only</div>
          </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-8">
          {activeTab === 'itinerary' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                  <div className="flex overflow-x-auto gap-3 pb-4 no-scrollbar snap-x">
                      {trip.dailyItinerary.map((day, idx) => (
                          <button key={idx} onClick={() => setActiveDay(idx)} className={clsx("flex-shrink-0 w-16 h-20 border rounded-none flex flex-col items-center justify-center transition-all snap-center", activeDay === idx ? "bg-black text-white border-black" : "bg-white text-gray-500 border-gray-200")}>
                              <span className="text-[11px] uppercase font-bold">{format(parseISO(day.date), 'EEE')}</span>
                  ...
                  <div className="flex justify-between items-center border-b pb-2">
                      ...
                      <button onClick={() => setViewMode(viewMode === 'list' ? 'map' : 'list')} className="text-xs border border-gray-200 px-3 py-1 rounded-none uppercase font-bold text-gray-500 hover:border-black transition-colors">{viewMode === 'list' ? "地圖" : "列表"}</button>
                  </div>
                  {viewMode === 'list' ? (
                          ...
                          <div key={b.id} className="border border-gray-100 rounded-none p-5 bg-white relative">
                              <div className="flex justify-between mb-4">
                                  <div className="flex items-center gap-3">
                                      ...
                                      <div><p className="text-xs text-gray-500 uppercase tracking-widest">{b.type}</p><p className="font-bold">{b.title}</p></div>
                                  </div>
                              ...
                              {b.details.fileUrl && (<button onClick={() => window.open(b.details.fileUrl, '_blank')} className="w-full py-2 bg-black text-white text-xs font-bold uppercase rounded-none">查看憑證</button>)}
                          </div>
          ...
          {activeTab === 'budget' && (
              <div className="space-y-6 animate-in fade-in">
                  <div className="bg-black text-white p-6 rounded-none border border-gray-200 flex justify-between items-center">
                      <div><p className="text-xs tracking-widest opacity-60 uppercase mb-1">Total Spent</p><h2 className="text-4xl font-bold font-serif">¥{trip.expenses.reduce((acc, e) => acc + e.amount, 0).toLocaleString()}</h2></div>
                  ...
                  <div className="space-y-1">
                      {trip.expenses.map(e => (
                          <div key={e.id} className="flex justify-between items-center p-4 border-b border-gray-50">
                              <div><p className="font-bold text-sm">{e.itemName}</p><p className="text-xs text-gray-500 uppercase tracking-wider">{e.date} • {e.category}</p></div>
          ...
          {activeTab === 'planning' && (
              <div className="space-y-10 animate-in fade-in">
                  <section><h2 className="text-xs font-bold tracking-widest uppercase border-b pb-2 mb-4 text-gray-500">行李清單</h2><div className="grid grid-cols-1 gap-2">{packingItems.map(p => <SharePlanItem key={p.id} item={p} members={trip.members} />)}</div></section>
                  <section><h2 className="text-xs font-bold tracking-widest uppercase border-b pb-2 mb-4 text-gray-500">待辦事項</h2><div className="grid grid-cols-1 gap-2">{todoItems.map(p => <SharePlanItem key={p.id} item={p} members={trip.members} />)}</div></section>
                  <section><h2 className="text-xs font-bold tracking-widest uppercase border-b pb-2 mb-4 text-gray-500">購物清單</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{shoppingItems.map(p => (<div key={p.id} className="border border-gray-100 rounded-none p-4 flex gap-4">{p.imageUrl && <img src={p.imageUrl} className="w-20 h-20 rounded-none object-contain bg-white border border-gray-100"/>}<div className="flex-1"><p className="font-bold text-sm">{p.text}</p><p className="text-xs text-gray-500 mt-1 uppercase">預算: ¥{p.estimatedCost || '--'}</p><p className="text-xs text-gray-500 uppercase">地點: {p.location || '未定'}</p></div></div>))}</div></section>
              </div>
          )}
      </div>
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 text-[11px] text-gray-400 tracking-[0.4em] uppercase font-bold">VM&apos;S BUILD • SHARED</div>
    </div>
  );
}
