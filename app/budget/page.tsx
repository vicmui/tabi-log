...
export default function BudgetPage() {
  ...
  const handleSave = () => {
    ...
    const expenseData = { amount: Number(amount), category, itemName, note, date: date || new Date().toISOString().split('T')[0], payerId: payer, splitWithIds: splitWith, customSplit: finalCustomSplit, receiptUrl };
  ...
  const debts = useMemo(() => {
      ...
      trip.expenses.forEach(exp => {
          ...
          if (exp.customSplit) { balances[paidBy] += exp.amount; Object.entries(exp.customSplit).forEach(([mid, amt]) => balances[mid] -= amt); } 
          else { const splitCount = exp.splitWithIds.length; if (splitCount === 0) return; const splitAmount = exp.amount / splitCount; balances[paidBy] += exp.amount; exp.splitWithIds.forEach(uid => balances[uid] -= splitAmount); }
      });
      const result = [];
      ...
      while (i < debtors.length && j < creditors.length) {
        ...
        if (Math.abs(debtors[i][1]) < 0.1) i++; if (creditors[j][1] < 0.1) j++;
      }
      return result;
  ...
  return (
    <div className="flex min-h-screen bg-white font-sans text-jp-charcoal">
      ...
      <main className="flex-1 ml-0 md:ml-64 p-8 md:p-12 overflow-y-auto h-screen bg-gray-50 pb-24">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {debts.length > 0 ? debts.map((d, idx) => (
                <div key={idx} className="bg-white p-6 border-l-4 border-jp-charcoal shadow-sm flex items-center justify-between">
                   <div className="flex items-center gap-2 text-sm"><span className="font-bold">{getMemberName(d.from)}</span><ArrowRight size={14} className="text-gray-400"/><span className="font-bold">{getMemberName(d.to)}</span></div>
        ...
        {/* 🔥 修正：使用 md:grid-cols-3 確保手機版垂直排列 */}
        ...
        {/* ... 新增支出和列表區塊保持不變 (因為已經是 responsive) ... */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
           {/* Add Expense Form */}
           <div className="bg-white p-8 shadow-sm h-fit border border-gray-100 sticky top-4">
              <div className="flex justify-between items-center mb-6"><h3 className="font-serif font-bold">{editingExpenseId ? "編輯" : "新增"}</h3>{editingExpenseId && <button onClick={()=>{setEditingExpenseId(null); setItemName(""); setAmount(""); setReceiptUrl("")}} className="text-xs text-gray-400">取消</button>}</div>
              <div className="space-y-4">
                 ...
                 <input type="number" placeholder="¥" value={amount} onChange={e=>setAmount(e.target.value)} className="w-full border-b p-2 text-xl" />
