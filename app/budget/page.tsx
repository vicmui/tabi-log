      ...
      <main className="flex-1 ml-0 md:ml-64 p-8 md:p-12 overflow-y-auto h-screen bg-gray-50 pb-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
           <div className="bg-white p-8 shadow-sm h-fit border border-gray-100 sticky top-4">
              <div className="flex justify-between items-center mb-6"><h3 className="font-serif font-bold">{editingExpenseId ? "編輯支出" : "新增支出"}</h3>{editingExpenseId && <button onClick={()=>{setEditingExpenseId(null); setItemName(""); setAmount(""); setReceiptUrl("")}} className="text-xs text-gray-400">取消</button>}</div>
              <div className="space-y-4">
                 <input type="date" value={date} onChange={e=>setDate(e.target.value)} className="w-full border-b p-2 text-sm" />
                 <input type="text" placeholder="項目名稱" value={itemName} onChange={e=>setItemName(e.target.value)} className="w-full border-b p-2 text-sm" />
           ...
           <div className="lg:col-span-2 space-y-8">
              ...
              <div className="space-y-2">{trip.expenses.map(exp => (<div key={exp.id} className="bg-white p-4 flex justify-between items-center border-b border-gray-50 hover:bg-gray-50 group"><div className="flex items-center gap-4"><div className="text-xs text-gray-400 font-mono w-20">{exp.date}</div><div><p className="font-bold text-sm flex items-center gap-2">{exp.itemName} {exp.receiptUrl && <a href={exp.receiptUrl} target="_blank" rel="noopener noreferrer"><Paperclip size={12} className="text-gray-400"/></a>}</p><p className="text-[10px] text-gray-400">{CAT_CONFIG[exp.category].label} • Paid by {trip.members.find(m=>m.id===exp.payerId)?.name} {exp.customSplit ? '(自訂)' : ''}</p></div></div><div className="flex items-center gap-4"><span className="font-serif font-bold">¥{exp.amount.toLocaleString()}</span><div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={()=>handleEditExpense(exp)} className="text-gray-400 hover:text-black"><Edit size={14}/></button><button onClick={()=>handleDelete(exp.id)} className="text-gray-400 hover:text-red-500"><Trash2 size={14}/></button></div></div></div>))}</div>
           </div>
        </div>
      </main>
    </div>
