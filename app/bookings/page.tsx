...
import { Plane, Building, Ticket, Car, MapPin, Download, Plus, X } from "lucide-react";
import { v4 as uuidv4 } from 'uuid';

export default function BookingsPage() {
  const { trips, activeTripId, addBooking } = useTripStore();
    ...
    <div className="flex min-h-screen bg-white font-sans text-jp-charcoal">
      ...
      <main className="flex-1 ml-0 md:ml-64 p-8 md:p-12 bg-gray-50 min-h-screen">
        <header className="mb-10 flex justify-between items-end">
          ...
          <button onClick={()=>setIsModalOpen(true)} className="bg-jp-charcoal text-white px-4 py-2 text-xs tracking-widest uppercase flex items-center gap-2 hover:bg-black">
             <Plus size={14}/> 新增預訂
          </button>
        </header>
...
function AddBookingModal({ onClose, onAdd }: any) {
  ...
  return (
     <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
        <div className="bg-white p-8 w-full max-w-md shadow-2xl relative">
           ...
           <h2 className="font-serif font-bold text-xl mb-6">新增預訂</h2>
           ...
           <button onClick={handleSubmit} className="w-full bg-black text-white py-3 text-xs uppercase tracking-widest hover:opacity-80">確認新增</button>
        </div>
     </div>
  )
}