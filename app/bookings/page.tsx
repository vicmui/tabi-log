...
import { Plane, Building, Ticket, Car, MapPin, Download, Plus, X, Edit, Trash2, CheckCircle2, Upload, ArrowRightLeft } from "lucide-react";
    ...
    <div className="flex min-h-screen bg-white font-sans text-jp-charcoal">
      ...
      <main className="flex-1 ml-0 md:ml-64 p-8 md:p-12 bg-gray-50 min-h-screen pb-24">
        <header className="mb-10 flex justify-between items-end">
          <div><h1 className="text-3xl font-serif font-bold tracking-widest uppercase mb-2">預訂憑證</h1><div className="flex items-center gap-4"><p className="text-xs text-gray-400 tracking-widest uppercase">Bookings & Tickets</p><span className="text-gray-300">|</span><TripSwitcher /></div></div>
        ...
        <div className="grid grid-cols-1 gap-6 max-w-3xl mx-auto">
          {trip.bookings && trip.bookings.length > 0 ? trip.bookings.map((booking) => (<BookingCard key={booking.id} booking={booking} onEdit={()=>handleEdit(booking)} onDelete={()=>handleDelete(booking.id)} />)) : <div className="text-gray-400 text-sm text-center py-20">此旅程暫無預訂資料</div>}
        </div>
  ...
  return (
    <div className="bg-white rounded-none shadow-sm border border-gray-200 overflow-hidden relative group hover:shadow-md transition-all rounded-xl">
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-3 gap-4 text-sm mb-4 pt-2 border-t border-dashed border-gray-200">{details.seat && (<div><p className="text-[9px] text-gray-400 uppercase">座位</p><p className="font-bold">{details.seat}</p></div>)}{details.gate && (<div><p className="text-[9px] text-gray-400 uppercase">登機門</p><p className="font-bold">{details.gate}</p></div>)}{details.address && (<div className="col-span-3 flex items-center gap-1 text-gray-600 bg-white p-2 border rounded"><MapPin size={14}/><span className="truncate">{details.address}</span></div>)}</div>
...
function BookingModal({ onClose, onSave, initialData, trip }: any) {
  ...
  const [inputPrice, setInputPrice] = useState(initDetails.price ? initDetails.price.toString() : ""); // 用戶輸入
  const [currency, setCurrency] = useState("JPY"); // JPY or HKD
  ...
  // 🔥 匯率換算邏輯
  const rate = trip?.exchangeRate || 0.052; // 0.052 HKD = 1 JPY => 1 HKD = 19.23 JPY
  
  const handleSubmit = () => {
    // 儲存時統一轉為 JPY
    let finalPrice = Number(inputPrice);
    if (currency === "HKD") {
        finalPrice = Math.round(Number(inputPrice) / rate);
    }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        const filePath = `public/${trip.id}/bookings/${fileName}`;
     ...
     <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
        <div className="bg-white p-6 w-full max-w-md shadow-2xl relative rounded-xl max-h-[90vh] overflow-y-auto">
           ...
           <div className="space-y-3">
               <input className="w-full border-b p-2 text-sm" placeholder="標題" value={title} onChange={e=>setTitle(e.target.value)}/>
               ...
               <div className="relative">
                   <div className="flex items-center border-b">
                       <input className="w-full p-2 text-sm focus:outline-none" type="number" placeholder="費用" value={inputPrice} onChange={e=>setInputPrice(e.target.value)}/>
                   ...
                   {currency === "HKD" && inputPrice && (
                       <p className="text-[10px] text-gray-400 mt-1 text-right">≈ ¥{Math.round(Number(inputPrice) / rate).toLocaleString()} (Rate: {rate})</p>
                   )}
               </div>
